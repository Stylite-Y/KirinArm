...
def Badminton():
    ...
    # 数据轨迹拟合
    zs = np.polyfit(t, ang_sm, 8)
    ze = np.polyfit(t, ang_em, 5)
    ps = np.poly1d(zs)
    pe = np.poly1d(ze)
    ang_sp = ps(t)
    ang_ep = pe(t)
    ...
    y1 = l*np.sin(ang_sa)
    x2 = -l*np.cos(ang_sa) - l*np.cos(ang_ea+ang_sa-np.pi)
    y2 = l*np.sin(ang_sa) + l*np.sin(ang_ea+ang_sa-np.pi)
    ...
    fig3, axs3 = plt.subplots(2, 1, figsize=(12, 12))
    ax2 = axs3[0]
    ax3 = axs3[1]

    ax2.scatter(t, ang_sm, color = 'C0')
    ax2.plot(t, ang_sp, color = 'C2', lw=2)
    ax3.scatter(t, ang_em, color = 'C0')
    ax3.plot(t, ang_ep, color = 'C2', lw=3)
